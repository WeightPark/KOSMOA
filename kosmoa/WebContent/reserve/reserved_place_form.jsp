...
System.out.println("place args : " + args);
PlaceInfo place = (PlaceInfo)request.getAttribute("placeInfo");
...
if (!isLogin) {
	...
	out.println("location.href='/kosmoa/member/loginForm.jsp?url=/kosmoa/place_view.pla" + ((args + "&piid=" + place.getPi_id()).replace("&", "$")) + "';");
	out.println("</script>");
	out.close();
}
...
function setRdate(rdate) {
	if (rdate < <%=curDay %>) {
		...
		document.frmPlaceReserve.rdate.value = rday;
	}
}
</script>
...
<div class="calendar-box">
   ...
   <tr height="30">
   <th width="80" class="txtRed">일</th><th width="80">월</th><th width="80">화</th><th width="80">수</th>
   <th width="80">목</th><th width="80">금</th><th width="80" class="txtBlue">토</th>
   </tr>
        <%
...
<div class="reserve-tool">
    <h2 style="margin-bottom: 30px;">장소 예약</h2>
    <form name="frmPlaceReserve" method="post" action="reserved_proc.res">
    <!-- request를 위한 hidden 객체 -->
    <input type="hidden" name="rtype" value="<%=rtype %>">
    <input type="hidden" name="rdate" value=""/>
    <input type="hidden" name="uid" value="<%=memberInfo.getPm_id() %>">
	<input type="hidden" name="pi_id" value="<%=place.getPi_id() %>"/>
    <input type="hidden" name="pi_name" value="<%=place.getPi_name() %>"/>
    ...
    <table class="reserveBrd" width="500px">
     ...
     <tr height="80px">
         <td style="font-weight: bold;" align="left">
         	<input type="hidden" name="piid" value="<%=place.getPi_id() %>"/>
         	<input type="hidden" name="piname" value="<%=place.getPi_name() %>">
         	<%=place.getPi_name() %>
         </td>
         <td class="tdTime">
...
<%
			String[] arrTime = place.getPi_time().split("~");
			String[] arrStime = arrTime[0].split(":");
			String[] arrEtime = arrTime[1].split(":");
...
<%
		for (int k = 1 ; k <= place.getPi_person() ; k++) {
%>
